...
import agentops
from flights import plan_flight
...
agentops.init(api_key=agent_ops_api_key, default_tags=["autogen-tool-example"])
...
def fetch_zillow_data(location: str = None, page: int = 1, status_type: str = None, home_type: str = None,
                      sort: str = None,
                      min_price: int = None, max_price: int = None, rent_min_price: int = None,
                      rent_max_price: int = None,
...
calendar_planner = autogen.ConversableAgent(
    name="CalendarPlanner",
    system_message="You are tasked with planning the timeline and logistics that the user can follow in their housing search and/or moving process. "
                   "Given your tasks, you will likely need to intervene at multiple points in the conversation."
                   "Based on the user input, you will provide the necessary information to the engineer so that they can determine what function calls to make to meet the user's queries/criteria--namely using the amadeus APIs."
                   "If the user's queries/criteria can't be found or determined, you must take user input again and retry."
                   "Your tasks are as follows: "
                   "During the housing search, if the user has decided on the top properties they are interested in from the list provided in the report, you will determine dates and times the user can book viewings/tours, based on the available time slots and the user's availability."
                   "If the user is required to fly out from their current location for the viewings/tours, you will search for and plan the flight, taking user input on flight preferences like prices, number of adults, and more into account."
                   "If the user has finished their housing search and decided on a property they want to move into and when they want to move in, you will also determine the dates and times the user can book a moving truck to move their belongings."
                   "If the user is required to fly out from their current location for the move, you will search for and plan the flight, taking user input on flight preferences like prices, number of adults, and more into account."
                   "The eventual goal is to hand this information to the writer for it to save the generated report." ,
    description="Calendar Planner. Plans the timeline and logistics of the user's schedule during their housing search and/or moving process.",
    llm_config=llm_config,
)
...
There will be several points in the conversation where you, as the engineer, will need to kick in, depending on the task given to you by others.
...
You may need to call the flights module's function plan_flight to query the Amadeus API to search for and fetch flight details matching the user's criteria. If you need to plan a flight, do not generate new code. Call the flights plan_flight function directly and display the arguments used. 
The `plan_flight` function accepts the following parameters, use only the necessary ones:
- origin=iata code of origin airport, 
- destination=iata code of destination airport,
- departure_date=day of departure in format 'yyyy-MM-dd'
- adults=number of adults to fly
- children=number of children to fly
- max_price=user's budget
...
writer = autogen.ConversableAgent(
    ...
    Please write a real estate report containing details on the real estate listing generated from the user's criteria and details on the logistics/timeline the user can follow to guide them in their search and moving process. 
    You take feedback from the user/admin and refine your report based on the information gathered and queried. The report should have the following structure.
    For the real estate listing generated from the user's criteria, include a photo that is representative of the property, and put the content in pseudo ```md``` code block.
    Also include (updating the report when necessary) a calendar schedule, in ```md``` code block, containing the logistics/timeline, based on the information gathered and queried, of the following:
    1) steps to take during the housing search
        - including what days and times the user can book viewings/tours of the top properties they are interested in, based on the available time slots and user's availability
        - and the details are of their flight (i.e. date, time, origin, destination, etc.) if they need to fly out from their current location for the viewings/tours.
    2) if the user has decided on a property and move-in date
        - the details are of their flight (i.e. date, time, origin, destination, etc.) if they need to fly out from their current location for the move. 
    Every time you generate a report, ALWAYS save it as a markdown file calling the save_markdown_file function and provide the file name at the very end of the conversation.
...
register_function(
    ...
    caller=engineer,  # The assistant agent can suggest calls to the calculator.
    executor=executor,  # The user proxy agent can execute the calculator calls.
    name="plan_flight",  # By default, the function name is used as the tool name.
    description="A tool that fetches plans flights",  # A description of the tool.
)
...
groupchat = autogen.GroupChat(
    agents=[user_proxy, engineer, writer, executor, planner, calendar_planner],
    messages=[],
    max_round=20,
)
